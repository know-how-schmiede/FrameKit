...
The integration demo (**0.7.2**) is available in [`fusion_addin/FrameKit`](fusion_addin/FrameKit). It provides three dialog tabs, saved defaults, and a frame made from demo solid profiles or imported DXF sections with optional shelves, followed by an automatic fit of all visible geometry in the viewport. Feet and casters can be selected as cylindrical placeholders; custom entries can be saved and deleted. Each frame carries a panel; the top panel can also sit above the posts without corner cutouts. Shelf count, individual heights, and panel thickness are configurable; blank heights are spaced automatically. See the [demo installation and test guide](docu/demo_installation.md) (German) for setup and current verification status. The features below describe the planned full scope.

New in **0.7.2 (S10)**: **FrameKit: CSV-Listen exportieren** offers a cut list, a bill of materials, or both. The BOM includes panels with corner cutouts, feet/casters and STEP brackets, with explicit placeholder and missing-fastener warnings. Automatic startup now defers STEP imports until Fusion initialization is complete. [Usage, startup change and verification](docu/stueckliste.md) (German). 154 local tests pass; the user confirmed successful export and Fusion startup without a crash.

New in **0.7.0 (S09)**: **FrameKit: Zuschnittliste exportieren** exports the selected frame’s profile cut list to CSV, grouping matching definitions, lengths and end treatments. Includes manufacturer metadata and part IDs, with German and international CSV formats. The edit command now uses the original icon with a pencil badge. [Usage and verification](docu/zuschnittliste.md) (German). 142 local tests pass; native Fusion export verification is pending.
...
Fix in **0.5.3**: all edit-command event handlers are registered when the command opens. After loading, they forward input, preview, validation and execution events to the editor. Version 0.5.2 did not resolve the missing preview for the user. 132 local tests pass; Fusion verification of this fix is pending.

Fix in **0.5.4**: all edit-dialog inputs are created when the command opens. Loading a frame populates existing controls; the original stays hidden while previewing. The user still saw a static, obscured preview in 0.5.3. 133 local tests pass; Fusion verification is pending.
