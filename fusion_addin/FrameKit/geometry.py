...
import adsk.core
import adsk.fusion
from .demo import members, panels, panel_outline, supports
from .version import __version__


def create_frame(design, values):
    layout = [(part, 'profile') for part in members(values)]
    layout.extend((part, 'panel') for part in panels(values))
    layout.extend((part, 'support') for part in supports(values))
    ...
    try:
        for index, ((name, origin, size), kind) in enumerate(layout, 1):
            ...
            if kind == 'support':
                ...
                sketch.sketchCurves.sketchCircles.addByCenterRadius(
                    adsk.core.Point3D.create(size[0] / 20, size[1] / 20, 0), size[0] / 20)
                component.attributes.add('FrameKit', 'placeholder', 'true')
            elif kind == 'panel':
